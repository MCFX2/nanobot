...
import { MMOSaveFile, MMOSkillSave, commitSaveData, getPlayerSave } from "./mmofile";
...
export function startSkill(prompt: ChatInputCommandInteraction, starter: SkillSuccessStarter)
{
    setTimeout(() => {
        const channel = prompt.channel;
        if(!channel) {
            mmoLogger.log("Invalid channel when trying to notify skill completion", WarningLevel.Error);
            return;
        }
...
export class BaseSkill {
    ...
    protected skillMaxedMessage = "You are already a master woodchopper! There's nothing more to learn. Retire and enjoy your golden years. Or try one of the other skills.";
    ...
    public doSkill(userId: string): SkillStarter {
        ...
        const xpToNextLevel = this.baseXp * Math.pow(this.xpScaleRate, skillSave.level - 1);

        const bonusLevels = save.inventory.reduce((acc, item) => {
            if(item.itemEffect === this.BetterBuff || item.itemEffect === ItemBuffType.EverythingBetter) {
                return acc + item.power;
            }
            return acc;
        }, 0);
        const adjustedBaseXp = this.baseXp * Math.pow(this.xpGainRate + 0.03 * bonusLevels, skillSave.level - 1);
        const awardedXp = Math.round(adjustedBaseXp + (Math.random() - 0.5) * (this.xpVariance * adjustedBaseXp));
        ...
        const speed = save.inventory.reduce((acc, item) => {
            if(item.itemEffect === this.FasterBuff || item.itemEffect === ItemBuffType.EverythingFaster) {
                return acc + item.power;
            }
            return acc;
        }, 0);
    
        const rate = Math.pow(2, speed);
    
        const message = `${this.beginPrompt} This will take about ${Math.round(this.baseTime / rate)} seconds.${
            luck > 45 ? " You feel especially lucky, for some reason."
            : luck < 5 ? " You feel like you're going to have a bad time."
                : ""}`;
        const levelUpMessage = `${this.levelUpPrompt} You are now level ${skillSave.level + 1} and your new title is **${this.ranks[skillSave.level]}**.`;
        ...
        if(loot) {
            ...
            {
                lootMessage = `\nCool! You got a **${loot.displayName}**!!! [${generateDescription(loot)}]`;
            }
        }
