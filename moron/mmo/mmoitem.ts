...
export const RandomEvents: MMORandomEvent[] = [
	{
		type: "event",
		impactText:
			"A tree falls on you! You've forgotten some of your woodchopping skills.",
	...
	},
	{
		type: "event",
		impactText:
			"A fish bites you! You've forgotten some of your fishing skills.",
		doEffect: (userId: string) => {
			const save = getPlayerSave(userId);
	...
	},
	{
		type: "event",
		impactText:
			"You fell on a rock. You've forgotten some of your mining skills.",
		doEffect: (userId: string) => {
			const save = getPlayerSave(userId);
	...
	},
	{
		type: "event",
		impactText:
			"You cut yourself on... something. You forgor some of your foraging skills.",
		doEffect: (userId: string) => {
			const save = getPlayerSave(userId);
	...
	},
	{
		type: "event",
		impactText:
			"You got in a fight with your own shadow. And lost. You've forgotten some of your combat skills.",
		doEffect: (userId: string) => {
			const save = getPlayerSave(userId);
	...
	},
	{
		type: "event",
		impactText:
			"Your axe broke, but you found out you can just use your hands. You've gained some woodchopping skills.",
		doEffect: (userId: string) => {
			const save = getPlayerSave(userId);
	...
	},
	{
		type: "event",
		impactText:
			"You've become much, much better at holding your breath. Consequently, your fishing skills have improved.",
		doEffect: (userId: string) => {
			const save = getPlayerSave(userId);
			save.fishing.xp += 150;
			commitSaveData();
	...
	},
	{
		type: "event",
		impactText:
			"You found an abandoned dwarf mine. You learned a lot about mining technique.",
		doEffect: (userId: string) => {
			const save = getPlayerSave(userId);
			save.mining.xp += 150;
			commitSaveData();
	...
	},
	{
		type: "event",
		impactText:
			"You found a book on foraging. Then you tried to eat it. But the book helpfully explained why that's a bad idea. Your foraging skills have improved.",
		doEffect: (userId: string) => {
			const save = getPlayerSave(userId);
	...
	},
	{
		type: "event",
		impactText:
			"You found a book on combat. You didn't understand a word of it, but you feel stronger anyway.",
		doEffect: (userId: string) => {
			const save = getPlayerSave(userId);
			save.combat.xp += 150;
			commitSaveData();
	...
	},
	{
		type: "event",
		impactText:
			"What the hell happened, man? Your skills seem to have noticeably degraded.",
		doEffect: (userId: string) => {
			const save = getPlayerSave(userId);
	...
	},
	{
		type: "event",
		impactText:
			"You ate a really, really, really bad sandwich. You didn't even know sandwiches could be that bad. Your skills have permanently decreased as a result of eating that sandwich.",
		doEffect: (userId: string) => {
			const save = getPlayerSave(userId);
			save.woodchop.xp = Math.max(0, save.woodchop.xp - 100);
			save.fishing.xp = Math.max(0, save.fishing.xp - 100);
	...
	},
	{
		type: "event",
		impactText:
			"You're feeling weirdly good about yourself. You've gained some skills in all areas.",
		doEffect: (userId: string) => {
			const save = getPlayerSave(userId);
			save.woodchop.xp += 75;
			save.fishing.xp += 75;
	...
	},
	{
		type: "event",
		impactText:
			"You found a magical troll, AND you solved his riddles three, four, five, and even six! In exchange, he improved your abilities across the board.",
		doEffect: (userId: string) => {
			const save = getPlayerSave(userId);
...
export function generateDrop(
	luck: number,
): MMOItem | MMORandomEvent | undefined {
