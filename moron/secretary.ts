...
function onInit() {
	...
	for (const o of readObjs) {
		const sched = new Schedule(o.id);
		schedules.push(sched);
		Object.assign(sched, o);
	}
...
class Schedule {
	...
	public IsFreeAtTime(
		...
		const result: { isFree: boolean; reason?: string } = {
			isFree: true,
			reason: undefined,
		};
...
export function setSchedule(interaction: ChatInputCommandInteraction) {
	...
	for (const schedule of scheduleRequests) {
		...
		if (day === undefined && !isWeekday && !isWeekend) {
			interaction.reply({
				content: `good point. however, minor spelling mistake. you lose.\n(syntax error: bad day of week on item #${idx})`,
				ephemeral: true,
			});
			return;
		}
		...
		const reason =
			plusPos !== -1
				? args[2].substring(plusPos + 1, args[2].length)
				: undefined;
	...
	if (!userSchedule) {
		...
		if (!userSchedule) {
			logger.log(
				"couldnt make a new schedule for some reason",
				WarningLevel.Error,
			);
			interaction.reply({
				content: "oof ow ouch that hurt\n(internal error)",
				ephemeral: true,
			});
			return;
		}
...
export function whoCanMakeIt(interaction: ChatInputCommandInteraction) {
	...
	if (!time.value) {
		interaction.reply({
			content: "you need to specify a time",
			ephemeral: true,
		});
		return;
	}
	...
	for (const s of schedules) {
		if (
			s.IsFreeAtTime(
				DaysOfWeek.Thursday,
				Number.parseInt(time.value.toString()),
			).isFree
		) {
			users.push(s.id);
		}
